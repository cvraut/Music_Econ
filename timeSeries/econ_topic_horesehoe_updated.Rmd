...
target_values = colnames(lda)[13:23]
target_values
```
...
econ_nrc_rjags <- function(target_name, overall_lag)
{
  ...
  model_reg.1 <- "model {
    ...
    "
    jags.data = list (
      Y = standardize(Y), Xmat = Xmat, n = length(Y), numEcon = 5, timelag = overall_lag + 1
    )
...
```{r}
target_values[5]
models <- econ_nrc_rjags(target_values[5], 3)
```

```{r}
jags.fit1 <- models
post.beta.mean <- jags.fit1$mean$beta
post.step.beta <- apply(jags.fit1$sims.list$beta, 2, function(a){sum(a>0)/length(a)})
